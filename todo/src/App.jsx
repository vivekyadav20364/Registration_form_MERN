...
const App = () => {
  ...
  const getdata = async () => {
    let response = await Axios.get("http://localhost:800/givenData");
  ...
  const handledelete = async (id) => {
    let response = await Axios.delete(`http://localhost:800/Delete/${id}`);
   // console.log(id);
    setItems(
  ...
  const handlersubmit = async (e) => {
    ...
    let response = await Axios.post("http://localhost:800/givenData", {
      Data: inputlist,
      Desp: task,
      Date: date,
    });
